...
const PasswordProtection = ({unlock}: PasswordProtectionProps) => {
    ...
    return (
        <form className="bg-white dark:bg-slate-700 rounded-xl p-5">
            <h2 className="text-slate-100 font-semibold text-lg mb-5">This questionnaire is password protected</h2>

            <div className="flex flex-col">
                <input value={password} onChange={(e) => setPassword(e.target.value) } id="password" placeholder="Questionnaire's password" name="password" type="password" className="bg-transparent text-slate-100 rounded-md border border-slate-200 outline-none py-1"/>
                <label htmlFor="password" className="text-xs text-slate-100 mt-2">Enter password to continue</label>
            </div>
