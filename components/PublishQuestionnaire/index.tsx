...
const PublishQuestionnaire = ({questionnaireId, callback}: { questionnaireId: number, callback?: () => void }) => {
    ...
    return (
        <Popover className="relative">
            <Popover.Button onClick={fetchStatus}
                            className="text-white text-sm py-1 px-2 font-semibold rounded-md focus:outline-none focus:ring-2  focus:ring-opacity-75 bg-green-500 hover:bg-green-700 focus:ring-green-400">
                Publish
            </Popover.Button>
            <Transition
                as={Fragment}
                ...
                <Popover.Panel className="absolute z-10 right-0">
                    <div className="rounded-md shadow-md p-3 bg-white w-60">
                        ...
                        <input value={name} onChange={(e) => setName(e.target.value)}
                               className="block text-sm py-1 px-2 w-full outline-none border border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500  rounded-md"/>
                        <div className="mt-5 w-full flex items-center justify-between">
                            {canPublish ? (
                                <>
                                    {lastPublish ? (
                                        <span
                                            className="text-xs font-medium">Last: {dayjs(lastPublish).format("DD/MM/YY H:mm")}</span>
                                    ) : (
                                        <span className="text-xs font-medium">First publish</span>
                                    )}
                                </>
                            ) : (
                            ...
                            <Popover.Button as={"div"}>
                                <Button type="success" className="py-0.5 px-1 text-sm" disabled={disabledButton}
