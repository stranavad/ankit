...
const IconButton = ({invert = false, icon: Icon, size = 'small', color = 'default', className = '', onClick, disabled = false}: IconButtonProps) => {
    ...
    return (
        <button className={className} disabled={disabled} onClick={onClick}>
            <Icon className={`${iconSize} ${disabled ? 'text-gray-600' : iconColor} ${disabled ? 'cursor-not-allowed' : hoverColor} transition-colors duration-75`}/>
        </button>
    )
}
